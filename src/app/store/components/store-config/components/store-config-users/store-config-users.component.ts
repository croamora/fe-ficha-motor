...
@Component({
  selector: 'app-store-config-users',
  standalone: true,
  imports: [
    DragDropModule,
    MatDialogModule,
  ...
  templateUrl: './store-config-users.component.html',
  styleUrl: './store-config-users.component.scss'
})
export class StoreConfigUsersComponent implements OnInit {
  ...
  public callData(event?: PageEvent): void {
    ...
    this.userService.getUsersStore(idTaller, pageNum, pageSize, this.palabraClave).subscribe({
      next: (response) => {
        this.spinner.hide();
